...
if($event->profile_picture){
    ...
    $imgURL = url('/').'/imgs/default-image.png';
      ...
      <a href="{{ url('/member')}}/{{ $event->UID }}"> <p class="adress text-dark user-age bg-light"><b>{{$event->title}}</b><a> - <b class="border rounded user-age">{{ $userAge }}</b></p>
     <?php $date = date( 'D, M d', strtotime( $event->created_at) );

     $event_date = date( 'D, M d', strtotime( $event->event_date) );
  ...
  <!--    <div class="age-para">
      ...
      <div class="visitor-box-blue d-flex justify-content-center mt-3">
       ...
       <div class="visitor-box-blue d-flex bg-white px-3">
        <!-- <button type="button" class="btn btn-lg-box1">Contact</button> -->
      <a href="{{ url('/member')}}/{{ $event->UID }}"> <span> <strong class="ml-2">View User Profile </strong></span> </a>
       <!-- <strong class="ml-2">{{ $event->username }}</strong> -->
    </div>
