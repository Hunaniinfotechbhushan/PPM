...
{   
    ...
    public function prepareConversationList($type,$specificUserId = null)
    {
        ...
        if(isset($_GET['uid'])){
            ...
            ChatModel::where('to_users__id', array_get($userDetails, 'profile._id'))->where('from_users__id', $_GET['uid'])->update(array('msg_status' => 1)); 

            $chatUserDetails = $this->getFullUserDetails($_GET['uid']);
        }else{
            $chatUserDetails = array();
        }
        ...
        if (\__isEmpty($specificUserId)) {            
            $userId = array_get($userDetails, 'profile._id');
            if($type=='sent'){
                $messengerUserCollection = $this->messengerRepository->fetchSentMessengerUsers($userId);

            }
            if($type=='inbox'){
              $messengerUserCollection = $this->messengerRepository->fetchInboxMessengerUsers($userId);  
          }
              ...
              $messengerUserCollection = $this->messengerRepository->fetchFilteredOutMessengerUsers($userId);  
                   ...
                   if($type=='archive'){
              $messengerUserCollection = $this->messengerRepository->fetchArchiveMessengerUsers($userId);  
          }

                 if($type=='deleted'){
              $messengerUserCollection = $this->messengerRepository->fetchDeletedMessengerUsers($userId);  
          }
...
if(empty($user_block_users)){
            $messengerUsers[] = [
                ...
                'user_id'           => $filter->users__id,
                'body_type_name'           => $filter->body_type_name,
                'ethnicity_name'           => $filter->ethnicity_name,
                'height'           => $filter->height,
                'userAge'       => $userAge,
                'countryName'   => $filter->countryName,
                'userOnlineStatus' => $this->getUserOnlineStatus($user->user_authority_updated_at),
                'userOnlineStatusAgo' => $this->getUserOnlineStatusAgo($user->user_authority_updated_at),
        ...
        $currentUserData = [
            'logged_in_user_full_name' => array_get($userDetails, 'profile.username'),
...
public function getBothUserConversation($userId)
{
...
$messageCollection = $this->messengerRepository->fetchBothUserConversations($userId);
...
if (!__isEmpty($messageCollection)) {
    foreach ($messageCollection as $messageChat) {
        ...
        $userConversations[] = [
            ...
            'message_from' => $messageChat->from_username,
            'message_to' => $messageChat->to_username,
            'users__id' => $messageChat->users__id,
            'is_message_received' => ($messageChat->message_from_user_id == $userId) ? true : false,
            'type'  => $messageChat->type,
        ];


        if($userId == $messageChat->users__id){
            $getUserDetails = UserProfile::where('users__id',$messageChat->users__id)->first();
            $getUserUid = User::where('_id',$messageChat->users__id)->first();
            $chatHistory .= '<div class="incoming_msg user-chat-text">
            <div class="incoming_msg_img"> <img src="'.url('/').'/media-storage/users/'.$getUserUid->_uid.'/'.$getUserDetails->profile_picture.'" alt="sunil"> </div>
            <div class="received_msg">
            <div class="received_withd_msg">
            <p>'.$messageChat->message.'</p>
            <span class="time_date"> '.Carbon::parse($messageChat->created_at)->format('g:i A j F').'</span></div>
            </div>
            </div>';
        }else{
         $chatHistory .= '<div class="outgoing_msg user-chat-text">
         <div class="sent_msg">
         <p>'.$messageChat->message.'</p>
         <span class="time_date"> '.Carbon::parse($messageChat->created_at)->format('g:i A | j F').'</span> </div>
         </div>';
...
return $userMessagesData = [
    ...
    public function prepareUserMessage($userId)
    {
        ...
        $userData = [
            ...
            'full_name' => $userDetails->username,
...
// Send Message form visitor Profile

    public function processSendMessageProfile($inputData, $userId)
    {
        ...
        $transactionResponse = $this->messengerRepository->processTransaction(function() use ($inputData, $userId) {
            ...
            // Check if user exists
            if (__isEmpty($userId)) {
                // return $responseData = array('error' => true,'msg'=>__tr('User does not exists.') );
              return $this->messengerRepository->transactionResponse(2, null, __tr('User does not exists.'));
          ...
          if (\__isEmpty($messageRequest)) {
            ...
            $initialMessageRequest = [
                ...
                // return $responseData = array('error' => false,'msg'=>__tr('Message has been sent') );

            $userDetailsEmail=getUserDetails($userDetails->_id);
                      
                        $urlmain =    URL::to('/');
                        $to = $userDetailsEmail->email;

                         //$to = 'webtest41@gmail.com';
                        $info = array(
                            'messagedata' => $inputData['msg'],
                            'username' => Auth::user()->username,
                            'url' => $urlmain,
                             'username' => $userDetailsEmail->username

                            
                        );
                        Mail::send('emails.messagesNote', $info, function ($message) use($to) {
                            $message->to($to, 'ppmarrangements')
                            ->subject('New messages');
                            $message->from('webtest41@gmail.com', 'ppmarrangements');
                        });

            return $this->messengerRepository->transactionResponse(1, null, __tr('Message has been sent.'));
        });

        return $this->engineReaction($transactionResponse);
    }

    public function sendChatMessage($inputData)
    {
       ...
       if($inputData){
          ...
          $initialMessageRequest = [
            [
                        ...
                        'type' => 9,
                        'from_users__id' => getUserID(),
                        'to_users__id' => $inputData['user_id'],
                        'users__id' => getUserID(),
                        'integrity_id' => ''
                    ]
                ];
                // Initial message request store in DB
                $savedMessage =  $this->messengerRepository->storeMessage($initialMessageRequest);

                $chatHistory = '<div class="outgoing_msg">
                <div class="sent_msg">
                <p>'.$inputData['message'].'</p>
                <span class="time_date"> '.Carbon::parse($savedMessage[0])->format('g:i A | j F').'</span> </div>
                </div>';
