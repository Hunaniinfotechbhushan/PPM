...
class UserEngine extends BaseEngine
{
	...
	public function processLogin($input)
	{
		...
		if ($user->is_verified == 5) {
			return $this->engineReaction(2, ['show_message' => true],  __tr('Your profile is disabled.'));
		}

		if ($user->is_verified == 3) {
			return $this->engineReaction(2, ['show_message' => true],  __tr('Your profile is deleted.'));
		}
      ...
      // Auth::attempt(['email' => $loginCredentials['email'], 'password' => $loginCredentials['password']]);
		...
		if (Auth::attempt($loginCredentials, $remember_me)) {
			...
			$UserIDTokan = User::find($user->_id);
			$UserIDTokan->recent_login = $ipdat->geoplugin_city.', '.$ipdat->geoplugin_countryName;
			$UserIDTokan->save(); 
            ...
            if ($this->userRepository->storeUserAuthority($userAuthorityData)) {
				...
				if ($this->userRepository->storeUserProfile($profileData)) {
					...
					if ($activationRequiredForNewUser) {
						...
						Auth::attempt(['email' => $inputData['email'], 'password' => $inputData['password']]);

						$Token = sha1(time());

						$UserIDTokan = User::find($newUser->_id);
...
return $this->engineReaction($transactionResponse);
    ...
    public function prepareUserProfile($userName)
    {
    	...
    	->select(
			...
			// 'occupation.name as user_occupation',
    	...
    	// ->join('occupation', 'occupation._id', '=', 'user_profiles.occupation')
...
// exp-profile
	...
	public function processUserLikeDislike($toUserUid, $like)
	{
		...
		//loggedIn user name
		$loggedInUserFullName = Auth::user()->username;
		...
		if (!__isEmpty($likeDislikeData)) {
			...
			if ($like == $likeDislikeData['like'] and $like == 1) {
				if ($this->userRepository->deleteLikeDislike($likeDislikeData)) {
					...
					$slug = 'user/dislike';
					$message = Auth::user()->username.' dislike your profile.';   
					$uid = $user->_uid;   
					$status = 1; 
			...
			if ($this->userRepository->updateLikeDislike($likeDislikeData, $updateData)) {
				...
				if ($like == 1) {
					...
					$slug = 'user/like';
					$message = Auth::user()->username.' like your profile.';   
					$uid = $user->_uid;   
					$status = 1; 
					$action = url('/').'/@'.$user->username;
					$is_read = 0;
...
// die;
	...
	public function finishWizard($inputData)
	{
		...
		if(isset($inputData['occupation'])){

			$ocption = $inputData['occupation'];
		}else{
			$ocption = '';
		...
		$userData["eye_color"] = $inputData['eye_color'];
		$userData["hair_color"] = $inputData['hair_color'];
		$userData["occupation"] = $ocption;
